...
def _get_default_font_family():
    """Get a default font family based on the operating system."""
...
def _load_font(name, size=8):
    ...
    if font_families:
        ...
        # Fallback to OS-appropriate default font if loading fails
        font_family = _get_default_font_family()
        logger.warning("Failed to load font '%s', using system default '%s'", name, font_family)
...
def _get_icon(icon, size=64, color="black"):
    ...
    if not op.exists(output_path):  # pragma: no cover
        ...
        draw.text((float(size - width) / 2, float(size - height) / 2), hex_icon, font=font, fill=color)
        ...
        draw_mask.text((float(size - width) / 2, float(size - height) / 2), hex_icon, font=font, fill=255)
