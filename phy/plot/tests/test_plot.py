...
@fixture
def x():
    return 0.25 * np.random.randn(1000)
...
def test_plot_grid(canvas, x, y):
    ...
    c[0, 0].plot(x=x, y=y)
    c[0, 1].hist(5 + x[::10])
    c[0, 2].scatter(x, y, color=np.random.uniform(0.5, 0.8, size=(1000, 4)))

    c[1, 0].lines(pos=[-1, -0.5, +1, -0.5])
    c[1, 1].text(pos=(0, 0), text='Hello world!', anchor=(0.0, 0.0))
    c[1, 1].polygon(pos=np.random.rand(5, 2))

    # Multiple scatters in the same subplot.
    c[1, 2].scatter(x[2::6], y[2::6], color=(0, 1, 0, 0.25), size=20, marker='asterisk')
    c[1, 2].scatter(x[::5], y[::5], color=(1, 0, 0, 0.35), size=50, marker='heart')
    c[1, 2].scatter(x[1::3], y[1::3], color=(1, 0, 1, 0.35), size=30, marker='heart')
...
def test_plot_stacked(qtbot, canvas):
    ...
    c[1].hist(np.random.rand(5, 10), color=np.random.uniform(0.4, 0.9, size=(5, 4)))
...
def test_plot_boxed(qtbot, canvas):
    ...
    c[2].hist(np.random.rand(5, 10), color=np.random.uniform(0.4, 0.9, size=(5, 4)))
...
def test_plot_uplot(qtbot, canvas):
    if isinstance(canvas, PlotCanvasMpl):
        # TODO: not implemented yet
        return
    x, y = 0.25 * np.random.randn(2, 1000)
