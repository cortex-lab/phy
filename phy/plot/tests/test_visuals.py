...
def test_scatter_custom(qtbot, canvas_pz):
    ...
    # Random position.
    pos = 0.2 * np.random.randn(n, 2)

    # Random colors.
    c = np.random.uniform(0.4, 0.7, size=(n, 4))
    c[:, -1] = 0.5
...
def test_uniform_scatter_custom(qtbot, canvas_pz):
    ...
    _test_visual(
        ...
        canvas_pz,
        UniformScatterVisual(
            color=_random_color() + (0.5,),
            size=10.0,
        ),
        pos=pos,
        masks=np.linspace(0.0, 1.0, n),
        data_bounds=None,
    )
...
def test_plot_empty(qtbot, canvas):
    y = np.zeros((1, 0))
    _test_visual(qtbot, canvas, PlotVisual(), y=y)
...
def test_plot_color(qtbot, canvas_pz):
    v = PlotVisual()
    canvas_pz.add_visual(v)
    data = v.validate(y=0.2 * np.random.randn(10), data_bounds='auto')
    assert v.vertex_count(**data) >= 0
    v.set_data(**data)
...
def test_plot_2(qtbot, canvas_pz):
    ...
    # Signal colors.
    c = np.random.uniform(0.5, 1, size=(n_signals, 4))
    c[:, 3] = 0.5

    # Depth.
    depth = np.linspace(0.0, -1.0, n_signals)
...
def test_uniform_plot_1(qtbot, canvas_pz):
    y = 0.2 * np.random.randn(10)
    _test_visual(qtbot, canvas_pz, UniformPlotVisual(), y=y, masks=0.5, data_bounds=NDC)


def test_uniform_plot_2(qtbot, canvas_pz):
    y = 0.2 * np.random.randn(10)
    _test_visual(qtbot, canvas_pz, UniformPlotVisual(), y=y, masks=0.5, data_bounds='auto')
...
def test_uniform_plot_list(qtbot, canvas_pz):
    ...
    _test_visual(
        qtbot, canvas_pz, UniformPlotVisual(color=(1.0, 0.0, 0.0, 1.0)), y=y, masks=[0.1, 0.9]
    )
...
def test_histogram_2(qtbot, canvas_pz):
    ...
    # Histogram colors.
    c = np.random.uniform(0.3, 0.6, size=(n_hists, 4))
    c[:, 3] = 1
...
def test_image_2(qtbot, canvas):
    n = 100
    _test_visual(
        qtbot, canvas, ImageVisual(), image=np.random.uniform(low=0.5, high=0.9, size=(n, n, 4))
    )
...
def test_line_0(qtbot, canvas_pz):
    ...
    color = np.random.uniform(0.5, 0.9, (n, 4))
...
def test_line_agg_geom_0(qtbot, canvas_pz):
    ...
    color = np.random.uniform(0.5, 0.9, 4)
    _test_visual(qtbot, canvas_pz, LineAggGeomVisual(), pos=P, color=color)
...
def test_plot_agg_empty(qtbot, canvas_pz):
    _test_visual(qtbot, canvas_pz, PlotAggVisual(), y=[])
...
def test_plot_agg_1(qtbot, canvas_pz):
    ...
    _test_visual(qtbot, canvas_pz, PlotAggVisual(closed=True), x=x, y=y, data_bounds='auto')
...
def test_plot_agg_2(qtbot, canvas_pz):
    ...
    color = np.random.uniform(low=0.5, high=0.9, size=(n_signals, 4))
...
def test_polygon_0(qtbot, canvas_pz):
    n = 9
    x = 0.5 * np.cos(np.linspace(0.0, 2 * np.pi, n))
    y = 0.5 * np.sin(np.linspace(0.0, 2 * np.pi, n))
    pos = np.c_[x, y]
...
def test_text_1(qtbot, canvas_pz):
    ...
    pos = np.c_[np.linspace(-0.5, 0.5, 10), np.linspace(-0.5, 0.5, 10)]
    color = np.ones((10, 4))
    color[:, 2] = 0

    _test_visual(qtbot, canvas_pz, TextVisual(font_size=32), pos=pos, text=text, color=color)
...
def test_text_2(qtbot, canvas_pz):
    ...
    text = ['12345'] * 5
    pos = [[0, 0], [-0.5, +0.5], [+0.5, +0.5], [-0.5, -0.5], [+0.5, -0.5]]
