...
Read more on framebuffers on OpenGL Wiki <https://www.opengl.org/wiki/Texture>
...
class Texture(GPUData, GLObject):
    ...
    _cpu_formats = {1: gl.GL_RED, 2: gl.GL_RG, 3: gl.GL_RGB, 4: gl.GL_RGBA}

    _gpu_formats = {1: gl.GL_RED, 2: gl.GL_RG, 3: gl.GL_RGB, 4: gl.GL_RGBA}

    _gpu_float_formats = {1: gl.GL_R32F, 2: gl.GL_RG32F, 3: gl.GL_RGB32F, 4: gl.GL_RGBA32F}
...
class Texture1D(Texture):
    ...
    def _setup(self):
        ...
        gl.glBindTexture(self.target, self._handle)
        gl.glTexImage1D(
            self.target, 0, self._gpu_format, self.width, 0, self._cpu_format, self.gtype, None
        )
        self._need_setup = False
    ...
    def _update(self):
        ...
        if self.need_update:
            ...
            for i, target in enumerate(targets):
                ...
                if pending is None:
                    continue
                if pending[1] < extents[0]:
                    continue
                if pending[0] > extents[1]:
                    continue
                ...
                height = nbytes // self.width
                gl.glTexSubImage2D(
                    target, 0, x, y, width, height, self._cpu_format, self.gtype, face
                )
