...
def cleanupCallback(context=None):
    ...
    def callback(context=None):  # ← Remove the function call from default
        """Clean up the context, assumes that the context will *not* render again!"""
        if context is None:  # ← Handle None case inside the function
...
def enable_depth_mask():
    ...
    glClearDepth(1.0)  # noqa

    glEnable(GL_BLEND)  # noqa
    glDepthRange(0.0, 1.0)  # noqa
